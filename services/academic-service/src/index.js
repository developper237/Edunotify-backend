  ...
  async (req, res) => {
    try {
      ...
      const requetes = await prisma.requeteNote.findMany({
        ...
        include: {
          etudiant: { select: { id: true, nom: true, prenom: true, matricule: true, email: true, filieres: true, classeEtudiant: { select: { nom: true, filiere: true } } } },
      ...
      for (const r of requetes) {
        ...
        if (!parEtudiant[eid]) {
          parEtudiant[eid] = {
            ...
            filiere:    r.etudiant.classeEtudiant?.filiere || '',
            classe:     r.etudiant.classeEtudiant?.nom || '',
            nbRequetes: 0,
            enAttente:  0,
            traitees:   0,
            rejetees:   0,
      ...
      return res.json({
        requetes: requetes.map(r => ({
          ...
          etudiant:     { id: r.etudiant.id, nom: r.etudiant.nom, prenom: r.etudiant.prenom, matricule: r.etudiant.matricule, filiere: r.etudiant.classeEtudiant?.filiere || '' },
    ...
    try {
      ...
      return res.json({
        etudiant: {
          ...
          matricule: etudiant.matricule, filiere: etudiant.classeEtudiant?.filiere || '',
          classe: etudiant.classeEtudiant?.nom || '', email: etudiant.email,
        },
        requetes: requetes.map(r => ({
          id:           r.id,
          statut:       r.statut,
